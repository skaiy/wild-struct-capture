...
- 优先检查原版 WAO 中已配置的 StructCapture Agent，并以短期工作负载 Bearer JWT 调用其通用 Agent chat。只有未配置 WAO 时才使用独立模型网关或本地资产驱动的整理路径；已配置的 WAO 不可用、认证失败或 chat 失败必须 fail-closed。
...
`extract` 与 `organize` 先同步构建本地知识包字段。完整配置 `WAO_BASE_URL` 及 OIDC 或 HS256 workload 认证后，BFF 为原版 WAO Agent 目录和 chat 请求发送 Bearer JWT，并以 `STRUCTCAPTURE_WAO_AGENT_ID`（默认名称为 `structcapture-organizer`）定位已安装的 StructCapture Agent。其 `business_domain` 必须为 `structcapture`，且目录的 tenant/project 必须为 `structcapture/default`。
...
生产演示使用 `AGENTOS_ENV=production`、`AGENTOS_AUTH_MODE=oidc`，并配置 `AGENTOS_OIDC_JWKS_URL`、`AGENTOS_OIDC_ISSUER` 与 `AGENTOS_OIDC_AUDIENCE`。Capture BFF 以 `STRUCTCAPTURE_WAO_OIDC_TOKEN_URL` 的 OAuth client-credentials 流程取得短期 access token；token 的 `iss`、`aud`、`sub`、`tenant_id=structcapture`、`project_id=default` 与未过期 `exp` 会先被 BFF 作配置/范围检查，再由 WAO 以 JWKS 验签作为最终权威。HS256-only 测试部署可用 `STRUCTCAPTURE_WAO_HS256_SECRET` 生成同一范围的短期 HS256 JWT，必要时附带可选的 `iss`/`aud`；完整 OIDC 配置始终优先。BFF 不把 tenant/project 写入 chat body 作为身份来源。

当 WAO 未配置时，服务端可同时配置 `STRUCTCAPTURE_LLM_BASE_URL` 与 `STRUCTCAPTURE_LLM_API_KEY`，BFF 会向该 OpenAI 兼容网关的 `/v1/chat/completions` 发送相同的 pack 证据。`STRUCTCAPTURE_LLM_MODEL` 默认 `deepseek-v4-flash`。两个远程增强都未配置时，home-inventory 仅填充可从 caption/direction 直接识别的品牌、规格、有效期、位置、数量和品类，其余字段为“待确认”，不转储整段说明。已配置 WAO 时，Agent 目录读取、token、scope 检查、chat、超时或回答解析失败都会返回 API 错误，不会回退。
...
- 不将 `STRUCTCAPTURE_WAO_HS256_SECRET` 暴露给浏览器、写入日志或提交仓库；它只用于 HS256-only 部署的服务端 workload token。
- 不用 `X-Identity`、chat body 或其他客户端字段伪造 tenant/project 的 verified isolation claims。

## DevStub 与故障回退

客户端继续使用同源 `HttpWaoClient("/api/wao")`，请求失败时回退为 DevStub，保证本地 POC 可演示。BFF 的本地整理不依赖 WAO：未配置有效的 `WAO_BASE_URL` 加认证时，才返回可审核的本地结果；已配置 WAO 后，运行时或 Agent 失败会由 BFF 作为错误返回。DevStub 仅是浏览器级兜底；它不是业务层的持久化替代方案。
