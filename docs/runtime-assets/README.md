...
知识包字段可使用 `enum`（或兼容的 `options`）定义可选值，并可为每个规范值添加
`synonyms`。例如 `synonyms: { "厨房柜": ["橱柜"] }` 会在证据明确时自动规范为
“厨房柜”；没有精确值或同义词匹配时，BFF 保留“待确认”，由整理页的人工逐项选择。

新增位置、车辆方向/位置或其他枚举时，请只修改对应知识包 JSON、递增 `version`，
并同步填写中文标签。Capture BFF 会读取这些运行时资产并规范化结果；不要将业务枚举
硬编码到 WAO 通用 Chat。
